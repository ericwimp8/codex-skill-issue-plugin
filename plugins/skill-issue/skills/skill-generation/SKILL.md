...
2. Confirm the destination, expected outcome, completion criteria, project context, target harness surface when known, whether the target harness requires host metadata, evaluated-agent model and reasoning level, unresolved implementation-time matters, generation viability, execution preference, authority boundary, and user-owned stop conditions.
...
- optional harness metadata that belongs in the harness-defined artifact rather than the canonical skill body.
...
7. When the confirmed target harness requires host metadata, read `references/harness-metadata.md` and create the harness-defined artifact. Do not create metadata the target harness does not support.

## Validate the Written Result

- Check frontmatter, naming, folder structure, reference paths, and resource necessity.
- When a harness-metadata artifact is present, check it against `references/harness-metadata.md`.
...
- the generated skill path, portable skill surface, and any generated harness metadata;
...
- the evaluated-agent model and reasoning level exactly as recorded by intake;
...
- `references/harness-metadata.md`: Harness host-metadata generation rules. Use only when the confirmed target harness requires host metadata.
