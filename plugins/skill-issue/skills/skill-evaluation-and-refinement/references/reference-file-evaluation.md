...
Give each task to a fresh independent agent using the campaign's recorded evaluated-agent configuration. Retain native evidence that the target skill loaded and that the intended reference file was opened before the answer or artifact was produced. Agent prose, output similarity, a path check, or evidence that only `SKILL.md` loaded does not prove reference use.
...
For every trial, retain the unmodified prompt, fresh-agent identity, effective model and reasoning level, target version, reference version, fixtures, native target-load evidence, native reference-open evidence, observable output, ground-truth comparison, result, and cleanup owner. Build the evidence record as described in `campaign-record.md`.
