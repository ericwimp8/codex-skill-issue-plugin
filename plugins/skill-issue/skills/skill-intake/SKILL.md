...
- Use the current workflow agent as-is; do not ask for or prescribe its model or reasoning level.
- Ask which model and reasoning level the evaluated agents should use. Explain that this configuration should match the intended use of the finished skill. Record the user's answer in their own terms without restricting or translating it.
...
- the evaluated-agent model and reasoning level in the user's own terms;
- the generation-viability assessment, selected execution preference, and authority boundary;
- the intended destination, target harness surface when known, and whether the target harness requires host metadata.
